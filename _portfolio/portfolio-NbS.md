...
One specific challenge in LST downscaling is that most super-resolution models are pretrained on standard RGB imagery, yielding features that transfer poorly to thermal infrared bands. Consequently, these models must often be trained from scratch, imposing a significant computational burden.Compounded by the immense scale of spatiotemporal datasets, this computational cost forces current studies to rely on discrete, clear-sky snapshots from specific dates and location, which severely limits the models' temporal generalization and spatial transferability across diverse regions. Furthermore, relying purely on optical inputs during training renders the architectures structurally inadequate for handling the extensive missing values caused by persistent cloud occlusion in real-world observations. 

Overcoming these barriers to reconstruct seamless, spatiotemporally continuous LST records remains a formidable yet profoundly significant challenge, motivating my strong interest in advancing this research direction.
